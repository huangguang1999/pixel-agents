...
import logoClaudeData from './logo-claude.json';
import logoCodexData from './logo-codex.json';
import logoGrokData from './logo-grok.json';

// ── Paletted pixel sprites ──────────────────────────────────────

interface PalettedSpriteJson {
  palette: Record<string, string>;
  pixels: string[][];
}

function resolvePalettedSprite(data: PalettedSpriteJson): SpriteData {
  return data.pixels.map((row) => row.map((key) => data.palette[key] ?? key));
}

/** Claude logo: 8x8 starburst in Anthropic orange, used inside floating agent labels */
export const LOGO_CLAUDE_SPRITE: SpriteData = resolvePalettedSprite(logoClaudeData);
/** Codex logo: 8x8 rosette ring in OpenAI teal */
export const LOGO_CODEX_SPRITE: SpriteData = resolvePalettedSprite(logoCodexData);
/** Grok logo: 8x8 bold X in xAI black */
export const LOGO_GROK_SPRITE: SpriteData = resolvePalettedSprite(logoGrokData);

/** Pick the agent-source logo sprite. Falls back to Claude for unknown sources. */
export function getLogoForSource(source: string | null | undefined): SpriteData {
  if (source === 'codex') return LOGO_CODEX_SPRITE;
  if (source === 'grok') return LOGO_GROK_SPRITE;
  return LOGO_CLAUDE_SPRITE;
}
...
// See floorTiles.ts: module-level sprite cache doesn't survive partial HMR.
