...
import { PAN_MARGIN_FRACTION } from '../../constants.js';
...
import { verbKeyFor } from '../engine/agentLabel.js';
...
export function OfficeCanvas({
  ...
  const { t } = useI18n();
  // The game loop runs on RAF, not React state; keep the label resolver in a
  // ref so language switches don't require rebuilding the loop.
  const getAgentLabelRef = useRef<(ch: Character) => string | null>(() => null);
  useEffect(() => {
    getAgentLabelRef.current = (ch: Character) => {
      if (ch.isSubagent) return null;
      return t.verbLabels[verbKeyFor(ch)];
    };
  }, [t]);
  ...
  const clampPan = useCallback(
    (px: number, py: number): { x: number; y: number } => {
      ...
      const effZoom = zoom * dpr;
      const mapW = layout.cols * TILE_SIZE * effZoom;
      const mapH = layout.rows * TILE_SIZE * effZoom;
    ...
    const stop = startGameLoop(canvas, {
      ...
      render: (ctx) => {
        ...
        const { offsetX, offsetY } = renderFrame(
          ...
          (ch) => getAgentLabelRef.current(ch),
        );
  ...
  const screenToWorld = useCallback(
    (clientX: number, clientY: number) => {
      ...
      const effZ = zoom * dpr;
      const worldX = (deviceX - offsetRef.current.x) / effZ;
      const worldY = (deviceY - offsetRef.current.y) / effZ;
  ...
  // Pinch-to-zoom and two-finger pan are both intentionally disabled — scene
  // stays fixed in the window. Swallow wheel events to prevent any browser
