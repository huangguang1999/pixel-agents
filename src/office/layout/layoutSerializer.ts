...
export function layoutToSeats(furniture: PlacedFurniture[]): Map<string, Seat> {
  ...
  for (const item of furniture) {
    ...
    let seatCount = 0;
    const bgRows = entry.backgroundTiles ?? 0;
    // Side-view sofas show a single profile — multiple tiles of height are
    // the sofa's depth on the floor, not extra seats. Skip the top rows so
    // only the bottom tile gets a seat (character sits aligned with cushion).
    // 'side' is the authored orientation; 'left'/'right' are the rotation-group
    // aliases used when placed as a rotation variant.
    const isSideOrientation =
      entry.orientation === 'side' ||
      entry.orientation === 'left' ||
      entry.orientation === 'right';
    const seatStartDr =
      isSideOrientation && entry.footprintH > 1 ? entry.footprintH - 1 : bgRows;
    for (let dr = seatStartDr; dr < entry.footprintH; dr++) {
      for (let dc = 0; dc < entry.footprintW; dc++) {
        ...
        // Front-facing chairs with a sprite shorter than a tile (e.g.
        // SOFA_FRONT: 16px sprite in 32px tile) leave empty floor below the
        // sprite. Anchor the seated character to the tile bottom so their
        // torso overlaps the sofa seat instead of floating at the backrest.
        const spriteH = entry.sprite.length;
        let renderYOffsetPx: number | undefined;
        if (entry.orientation === 'front' && spriteH < TILE_SIZE) {
          renderYOffsetPx = TILE_SIZE - spriteH;
        }
        ...
        seats.set(seatUid, {
          ...
          facingDir,
          assigned: false,
          ...(renderYOffsetPx !== undefined ? { renderYOffsetPx } : {}),
        });
        seatCount++;
      }
    }
