...
/** When the tile ahead is occupied, an agent pauses for up to this long
 *  waiting for it to clear. Past the threshold the agent drops its path
 *  so the FSM re-plans — prevents head-on deadlocks between two walkers
 *  staring each other down forever. Tuned so short crossings (another
 *  agent momentarily passing through) don't trigger a re-path. */
export const WALK_STALL_MAX_SEC = 1.5;
